...
import { useUpdateIssueStatusMutation, useGetIssueByIdQuery } from "@/app/redux/features/issues/issueApiSlice";
  ...
  return (
    <div className="flex flex-col w-[35vw] max-h-[80vh] overflow-auto no-scrollbar gap-3">
      <div className="flex items-center justify-between">
        <h1 className="text-xl text-wrap font-medium text-black-60">{issue.title}</h1>
      ...
      <div className="flex flex-col gap-2">
        ...
        <input id="description" type="text" className="text-black-60 font-medium rounded-lg border-black-20" disabled value={issue.description} />
      </div>
      ...
      <div className="grid grid-cols-2 w-full gap-3">
        ...
        <span className="justify-self-end text-sm text-black-40 items-center self-center">{issue.date_time}</span>
      </div>
      {issue.status != 4 && (
        <div className="flex items-center justify-end mt-5 gap-4">
          <OutlineButton label="Cancel Request" type="Normal" bg={"border-red text-red hover:text-red/50 hover:border-red/50"} onClick={() => handleuUpdateIssueStatus(4)} />
        </div>
      )}
    </div>
  );
