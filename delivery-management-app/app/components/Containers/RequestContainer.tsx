...
//////
import List, { ListSkeleton } from "../Lists/UserRequestList";
import { useGetCurrentEmployeeIssuesQuery } from "@/app/redux/features/issues/issueApiSlice";

export const Request = () => {
  const { data: requests, error, isLoading } = useGetCurrentEmployeeIssuesQuery(undefined);
  if (isLoading) {
    return (
      <div className="flex flex-1 flex-col bg-white p-3 gap-3">
        <div className="flex items-stretch justify-between p-3" aria-label="Global">
          <div className="flex-1 w-32 text-lg text-black-60 font-medium">Loading...</div>
        </div>
